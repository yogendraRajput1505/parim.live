<%@ page import="hibernate.HibernateUtil,org.hibernate.Session,java.util.List,org.hibernate.Query,database.Enquiries,org.hibernate.cfg.Configuration,
				...
				<table id="dtMaterialDesignExample" class="table table-responsive" cellspacing="0" width="100%">
				  ...
				  <%
					...
					//  Session ses = HibernateUtil.getSessionFactory().openSession();
			          ...
			          ses = sf.openSession();
			          Query userQuery  = ses.createQuery("from "+Enquiries.class.getName());
				          
				      List rows = userQuery.list();
					  Enquiries sd = null;
						for(int i=0;i<rows.size();i++){
					    	sd=(Enquiries) rows.get(i);%>
							<tr>
						      <td><%=sd.getId() %></td>
						      <td><%=sd.getName() %></td>
						      <td><%=sd.getEmail() %></td>
            			...
            			<% }
            				...
            				e.printStackTrace();
            				response.sendRedirect("../errorPages/technicalError.jsp");
            			}
				  		finally{
				  			if(ses!=null){
				  				ses.close();
